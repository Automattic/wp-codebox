...
static void sha_block(sha256 *s, const unsigned char *p) {
    ...
    for (;i<64;i++) { uint32_t x=w[i-15],y=w[i-2]; w[i]=(rr(x,7)^rr(x,18)^(x>>3))+w[i-16]+(rr(y,17)^rr(y,19)^(y>>10))+w[i-7]; }
    a=s->h[0];b=s->h[1];c=s->h[2];d=s->h[3];e=s->h[4];f=s->h[5];g=s->h[6];h=s->h[7];
    for(i=0;i<64;i++){ uint32_t s1=rr(e,6)^rr(e,11)^rr(e,25),ch=(e&f)^((~e)&g),t1=h+s1+ch+k[i]+w[i],s0=rr(a,2)^rr(a,13)^rr(a,22),maj=(a&b)^(a&c)^(b&c),t2=s0+maj; h=g;g=f;f=e;e=d+t1;d=c;c=b;b=a;a=t1+t2; }
...
static void sha_final(sha256 *s,char out[65]){unsigned char pad[128]={0x80},digest[32];uint64_t bits=s->bits;size_t n=s->used<56?56-s->used:120-s->used;sha_add(s,pad,n);for(int i=0;i<8;i++)pad[i]=(unsigned char)(bits>>(56-i*8));sha_add(s,pad,8);for(int i=0;i<8;i++){digest[i*4]=s->h[i]>>24;digest[i*4+1]=s->h[i]>>16;digest[i*4+2]=s->h[i]>>8;digest[i*4+3]=s->h[i];}for(int i=0;i<32;i++)sprintf(out+i*2,"%02x",digest[i]);out[64]=0;}

typedef struct { dev_t dev; ino_t ino; } identity;
static int same(identity a, identity b) { return a.dev==b.dev && a.ino==b.ino; }
static identity identity_of(struct stat st) { identity x={st.st_dev,st.st_ino}; return x; }
...
static int manifest_dir(int fd,const char *prefix,sha256 *hash) {
    ...
    for(size_t i=0;i<count;i++) {
        struct stat st; int child; char path[4096],header[8192];
        if(snprintf(path,sizeof(path),"%s%s%s",prefix,*prefix?"/":"",names[i])>=(int)sizeof(path))goto done;
        if(fstatat(fd,names[i],&st,AT_SYMLINK_NOFOLLOW)!=0 || S_ISLNK(st.st_mode))goto done;
    ...
    if(fstat(fd,&after)==0&&stable_metadata(before,after))result=0;
done:
...
static int copy_snapshot_dir(int source,int destination) {
    ...
    while((entry=readdir(dir))) {
        ...
        if(S_ISDIR(path_stat.st_mode)) {
            ...
            if(input<0||output<0||fstat(input,&opened)!=0||!S_ISDIR(opened.st_mode)||!stable_metadata(path_stat,opened)||fchmod(output,path_stat.st_mode&07777)!=0||copy_snapshot_dir(input,output)!=0||fstat(input,&source_after)!=0||!stable_metadata(opened,source_after)||fstatat(source,entry->d_name,&source_after,AT_SYMLINK_NOFOLLOW)!=0||!stable_metadata(opened,source_after)){if(input>=0)close(input);if(output>=0)close(output);goto done;}
            ...
            if(input<0||output<0||fstat(input,&opened)!=0||!S_ISREG(opened.st_mode)||!stable_metadata(path_stat,opened)||fchmod(output,path_stat.st_mode&07777)!=0){if(input>=0)close(input);if(output>=0)close(output);goto done;}
            while((got=read(input,buffer,sizeof(buffer)))>0){ssize_t sent=0;while(sent<got){ssize_t n=write(output,buffer+sent,(size_t)(got-sent));if(n<=0){close(input);close(output);goto done;}sent+=n;}}
            if(got<0||fsync(output)!=0||fstat(input,&source_after)!=0||!stable_metadata(opened,source_after)||fstatat(source,entry->d_name,&source_after,AT_SYMLINK_NOFOLLOW)!=0||!stable_metadata(opened,source_after)){close(input);close(output);goto done;}
            close(input);close(output);
        } else goto done;
    }
    {const char *barrier=getenv("WP_CODEBOX_SNAPSHOT_BARRIER");if(barrier&&*barrier){int marker=open(barrier,O_CREAT|O_EXCL|O_WRONLY|O_NOFOLLOW,0600);if(marker>=0){close(marker);while(access(barrier,F_OK)==0)usleep(1000);}}}
    if(fstat(source,&source_after)==0&&stable_metadata(source_before,source_after)&&sync_directory(destination)==0)result=0;
done:
    closedir(dir);return result;
}

static int split_path(const char *path,char **parent,char **name) {
    char *copy=strdup(path),*slash;if(!copy)return -1;slash=strrchr(copy,'/');
    if(!slash||!slash[1]){free(copy);return -1;}*name=strdup(slash+1);if(slash==copy)slash[1]=0;else *slash=0;*parent=copy;return *name?0:-1;
}
static int open_absolute_directory(const char *path) {
    if(!path||path[0]!='/')return -1;int fd=open("/",O_RDONLY|O_DIRECTORY|O_NOFOLLOW);char *copy=strdup(path),*save=NULL,*part;
    if(fd<0||!copy){if(fd>=0)close(fd);free(copy);return -1;}
    for(part=strtok_r(copy,"/",&save);part;part=strtok_r(NULL,"/",&save)){if(!strcmp(part,".")||!strcmp(part,"..")){close(fd);free(copy);return -1;}int next=openat(fd,part,O_RDONLY|O_DIRECTORY|O_NOFOLLOW);close(fd);if(next<0){free(copy);return -1;}fd=next;}
    free(copy);return fd;
}
static int open_relative_file(int root,const char *path) {
    if(!path||!*path||path[0]=='/')return -1;int fd=dup(root);char *copy=strdup(path),*save=NULL,*part,*next;
    if(fd<0||!copy){if(fd>=0)close(fd);free(copy);return -1;}
    part=strtok_r(copy,"/",&save);while(part){if(!strcmp(part,".")||!strcmp(part,"..")){close(fd);free(copy);return -1;}next=strtok_r(NULL,"/",&save);int opened=openat(fd,part,O_RDONLY|O_NOFOLLOW|(next?O_DIRECTORY:0));close(fd);if(opened<0){free(copy);return -1;}fd=opened;part=next;}
    free(copy);struct stat st;if(fstat(fd,&st)||!S_ISREG(st.st_mode)){close(fd);return -1;}return fd;
}
static int relative_file_identity(int root,const char *path,identity expected) {
    int file=open_relative_file(root,path);struct stat st;if(file<0)return 0;
    int result=fstat(file,&st)==0&&same(identity_of(st),expected);close(file);return result;
}
static int entry_identity(int parent,const char *name,identity expected) {
    struct stat st;return fstatat(parent,name,&st,AT_SYMLINK_NOFOLLOW)==0&&S_ISDIR(st.st_mode)&&!S_ISLNK(st.st_mode)&&same(identity_of(st),expected);
}
static int any_entry_identity(int parent,const char *name,identity expected) {
    struct stat st;return fstatat(parent,name,&st,AT_SYMLINK_NOFOLLOW)==0&&same(identity_of(st),expected);
}
...
typedef struct { int file; int parent; char *name; identity directory; } secure_lock_handle;
static secure_lock_handle secure_lock(const char *path) {
    secure_lock_handle result={-1,-1,NULL,{0,0}};struct stat st,after,directory;int fd;char *parent_path=NULL;
    if(split_path(path,&parent_path,&result.name))return result;
    result.parent=open_absolute_directory(parent_path);free(parent_path);if(result.parent<0)return result;
    if(mkdirat(result.parent,result.name,0700)==0){if(sync_directory(result.parent)!=0)return result;}else if(errno!=EEXIST)return result;
    fd=openat(result.parent,result.name,O_RDONLY|O_DIRECTORY|O_NOFOLLOW);if(fd<0)return result;
    if(fstat(fd,&directory)!=0||directory.st_uid!=geteuid()){close(fd);return result;}
    if((directory.st_mode&0777)!=0500&&fchmod(fd,0500)!=0){close(fd);return result;}
    if(fstat(fd,&directory)!=0||directory.st_uid!=geteuid()||(directory.st_mode&0777)!=0500){close(fd);return result;}
    int lock=openat(fd,"lock",O_CREAT|O_RDWR|O_NOFOLLOW,0600);
    if(lock<0 && errno==EACCES){if(fchmod(fd,0700)!=0){close(fd);return result;}lock=openat(fd,"lock",O_CREAT|O_RDWR|O_NOFOLLOW,0600);if(fchmod(fd,0500)!=0){if(lock>=0)close(lock);close(fd);return result;}}
    if(lock<0||fstat(lock,&st)!=0||!S_ISREG(st.st_mode)||st.st_uid!=geteuid()||(st.st_mode&0777)!=0600||flock(lock,LOCK_EX)!=0){if(lock>=0)close(lock);close(fd);return result;}
    if(fstatat(fd,"lock",&after,AT_SYMLINK_NOFOLLOW)!=0||!same(identity_of(st),identity_of(after))){close(lock);close(fd);return result;}
    result.file=lock;result.directory=identity_of(directory);close(fd);return result;
}
static int lock_continuous(secure_lock_handle lock) {
    ...
    if(directory_fd<0||fstat(directory_fd,&directory)||!same(identity_of(directory),lock.directory)||(directory.st_mode&0777)!=0500||directory.st_uid!=geteuid()){if(directory_fd>=0)close(directory_fd);return 0;}
    if(fstat(lock.file,&file)||fstatat(directory_fd,"lock",&path_file,AT_SYMLINK_NOFOLLOW)||!same(identity_of(file),identity_of(path_file))||!S_ISREG(file.st_mode)||file.st_uid!=geteuid()||(file.st_mode&0777)!=0600){close(directory_fd);return 0;}
...
static enum cleanup_result remove_child(int parent,const char *name,struct stat before,unsigned long sequence) {
    char temporary[128];struct stat st;identity expected=identity_of(before),placeholder;int child=-1;
    snprintf(temporary,sizeof(temporary),".wp-codebox-child-%ld-%lu",(long)getpid(),sequence);
    if(S_ISDIR(before.st_mode)) {
        if(mkdirat(parent,temporary,0700)!=0||sync_directory(parent)!=0)return CLEAN_PENDING;
    } else {
        int file=openat(parent,temporary,O_CREAT|O_EXCL|O_WRONLY|O_NOFOLLOW,0600);
        if(file<0)return CLEAN_PENDING;
        close(file);if(sync_directory(parent)!=0)return CLEAN_PENDING;
    }
    if(fstatat(parent,temporary,&st,AT_SYMLINK_NOFOLLOW)!=0)goto pending;
    placeholder=identity_of(st);
    if(exchange_at(parent,name,parent,temporary)!=0||sync_directory(parent)!=0)goto pending;
    if(fstatat(parent,temporary,&st,AT_SYMLINK_NOFOLLOW)!=0)goto pending;
    if(!same(identity_of(st),expected)) {
        if(any_entry_identity(parent,name,placeholder))exchange_at(parent,name,parent,temporary);
        goto pending;
    }
    if(S_ISDIR(before.st_mode)) {
        child=openat(parent,temporary,O_RDONLY|O_DIRECTORY|O_NOFOLLOW);
        if(child<0||fstat(child,&st)!=0||!same(identity_of(st),expected)||remove_contents(child)!=CLEAN_OK||fstat(child,&st)!=0||!same(identity_of(st),expected)){if(child>=0)close(child);goto restore;}
        close(child);child=-1;
        if(!entry_identity(parent,temporary,expected)||unlinkat(parent,temporary,AT_REMOVEDIR)!=0||sync_directory(parent)!=0)goto restore;
    } else {
        child=openat(parent,temporary,O_RDONLY|O_NOFOLLOW|O_NONBLOCK);
        if(child<0||fstat(child,&st)!=0||!same(identity_of(st),expected)){if(child>=0)close(child);goto restore;}
        close(child);child=-1;
        if(fstatat(parent,temporary,&st,AT_SYMLINK_NOFOLLOW)!=0||!same(identity_of(st),expected)||unlinkat(parent,temporary,0)!=0||sync_directory(parent)!=0)goto restore;
    }
    if(S_ISDIR(before.st_mode)) {
        if(!entry_identity(parent,name,placeholder)||unlinkat(parent,name,AT_REMOVEDIR)!=0||sync_directory(parent)!=0)return CLEAN_PENDING;
    } else {
        if(fstatat(parent,name,&st,AT_SYMLINK_NOFOLLOW)!=0||!same(identity_of(st),placeholder)||unlinkat(parent,name,0)!=0||sync_directory(parent)!=0)return CLEAN_PENDING;
    }
    return CLEAN_OK;
restore:
    if(child>=0)close(child);
    if(S_ISDIR(before.st_mode)) {
        if(entry_identity(parent,name,placeholder)&&entry_identity(parent,temporary,expected)&&exchange_at(parent,name,parent,temporary)==0)sync_directory(parent);
    } else if(any_entry_identity(parent,name,placeholder)&&any_entry_identity(parent,temporary,expected)&&exchange_at(parent,name,parent,temporary)==0)sync_directory(parent);
pending:
    if(S_ISDIR(before.st_mode)) {
        if(entry_identity(parent,temporary,placeholder)&&unlinkat(parent,temporary,AT_REMOVEDIR)==0)sync_directory(parent);
    } else if(fstatat(parent,temporary,&st,AT_SYMLINK_NOFOLLOW)==0&&same(identity_of(st),placeholder)&&unlinkat(parent,temporary,0)==0)sync_directory(parent);
    return CLEAN_PENDING;
}

static enum cleanup_result remove_contents(int fd) {
    static unsigned long sequence;int scan=openat(fd,".",O_RDONLY|O_DIRECTORY|O_NOFOLLOW);DIR *dir;struct dirent *e;char **names=NULL;size_t count=0;enum cleanup_result result=CLEAN_OK;
    if(scan<0||!(dir=fdopendir(scan))){if(scan>=0)close(scan);return CLEAN_PENDING;}
    while((e=readdir(dir))) {
        if(!strcmp(e->d_name,".")||!strcmp(e->d_name,".."))continue;
        char **grown=realloc(names,(count+1)*sizeof(*names));if(!grown){result=CLEAN_PENDING;break;}names=grown;
        names[count]=strdup(e->d_name);if(!names[count]){result=CLEAN_PENDING;break;}count++;
    }
    closedir(dir);
    if(result==CLEAN_OK)for(size_t i=0;i<count;i++){struct stat before;if(fstatat(fd,names[i],&before,AT_SYMLINK_NOFOLLOW)!=0||remove_child(fd,names[i],before,sequence++)!=CLEAN_OK){result=CLEAN_PENDING;break;}}
    for(size_t i=0;i<count;i++)free(names[i]);free(names);return result;
}
...
static int write_cleanup_state(int qfd,const char *token,identity expected,identity placeholder) {
    ...
    close(fd);return ok&&sync_directory(qfd)==0?0:-1;
}
static int read_cleanup_state(int qfd,const char *token,identity expected,identity *placeholder) {
    char data[256],saved[81],extra;unsigned long long ed,ei,pd,pi;int fd=openat(qfd,"owner",O_RDONLY|O_NOFOLLOW),result=0;struct stat st;ssize_t n;
    if(fd<0||fstat(fd,&st)||!S_ISREG(st.st_mode)||(st.st_mode&0777)!=0600||(n=read(fd,data,sizeof(data)-1))<=0){if(fd>=0)close(fd);return 0;}close(fd);data[n]=0;
    if(sscanf(data,"%80s %llu %llu %llu %llu %c",saved,&ed,&ei,&pd,&pi,&extra)==5&&!strcmp(saved,token)&&(dev_t)ed==expected.dev&&(ino_t)ei==expected.ino){placeholder->dev=(dev_t)pd;placeholder->ino=(ino_t)pi;result=1;}return result;
}
static int directory_empty(int fd) {
    int scan=openat(fd,".",O_RDONLY|O_DIRECTORY|O_NOFOLLOW),empty=0;DIR *dir;struct dirent *entry;if(scan<0||!(dir=fdopendir(scan))){if(scan>=0)close(scan);return 0;}
    empty=1;while((entry=readdir(dir)))if(strcmp(entry->d_name,".")&&strcmp(entry->d_name,"..")){empty=0;break;}closedir(dir);return empty;
}
static enum cleanup_result cleanup_entry(int parent,const char *name,identity expected,const char *token) {
    char quarantine[256];int qfd=-1,root=-1,created=0;struct stat st;identity placeholder={0,0};enum cleanup_result result=CLEAN_PENDING;
    if(!valid_token(token))return CLEAN_PENDING;
    snprintf(quarantine,sizeof(quarantine),".wp-codebox-cleanup-%llu-%llu-%s",(unsigned long long)expected.dev,(unsigned long long)expected.ino,token);
    ...
    if(entry_identity(qfd,"root",placeholder)) {
        if(!entry_identity(parent,name,expected)){
            if(unlinkat(qfd,"root",AT_REMOVEDIR)==0&&sync_directory(qfd)==0&&unlinkat(qfd,"owner",0)==0&&sync_directory(qfd)==0){close(qfd);qfd=-1;if(unlinkat(parent,quarantine,AT_REMOVEDIR)==0)sync_directory(parent);}
            return CLEAN_IDENTITY;
        }
        cleanup_barrier("before-exchange");
        if(exchange_at(parent,name,qfd,"root")!=0||sync_directory(parent)!=0||sync_directory(qfd)!=0)goto done;
    }
    if(!entry_identity(qfd,"root",expected)) {
        if(fstatat(qfd,"root",&st,AT_SYMLINK_NOFOLLOW)!=0&&errno==ENOENT&&unlinkat(qfd,"owner",0)==0&&sync_directory(qfd)==0){close(qfd);qfd=-1;if(unlinkat(parent,quarantine,AT_REMOVEDIR)==0&&sync_directory(parent)==0)return CLEAN_OK;}
        goto done;
    }
    ...
    result=remove_contents(root);close(root);root=-1;if(result!=CLEAN_OK)goto done;
    if(!entry_identity(qfd,"root",expected)||unlinkat(qfd,"root",AT_REMOVEDIR)!=0||sync_directory(qfd)!=0)goto done;
    if(unlinkat(qfd,"owner",0)!=0||sync_directory(qfd)!=0)goto done;
    if(unlinkat(parent,quarantine,AT_REMOVEDIR)!=0||sync_directory(parent)!=0)goto done;
    close(qfd);return CLEAN_OK;
done:
...
int main(int argc,char **argv) {
    ...
    if(argc==6&&!strcmp(argv[1],"--snapshot")) {
        int source=open_absolute_directory(argv[2]),destination=open_absolute_directory(argv[3]);char digest[65];struct stat before,after;identity expected={(dev_t)strtoull(argv[4],NULL,10),(ino_t)strtoull(argv[5],NULL,10)};
        if(source<0||destination<0||fstat(source,&before)||!same(identity_of(before),expected)||copy_snapshot_dir(source,destination)!=0||fstat(source,&after)||!stable_metadata(before,after)||manifest(destination,digest)!=0){if(source>=0)close(source);if(destination>=0)close(destination);write_status("INVALID_STAGING");return 6;}
        dprintf(STDOUT_FILENO,"SNAPSHOT %s\n",digest);close(source);close(destination);return 0;
    }
    if(argc==6&&!strcmp(argv[1],"--read")) {
        int root=open_absolute_directory(argv[2]);int file=root<0?-1:open_relative_file(root,argv[3]);unsigned char buffer[65536];ssize_t got;
        struct stat root_before,root_after,file_before,file_after;identity expected={(dev_t)strtoull(argv[4],NULL,10),(ino_t)strtoull(argv[5],NULL,10)};
        if(root<0||fstat(root,&root_before)||!same(identity_of(root_before),expected)||file<0||fstat(file,&file_before)||!S_ISREG(file_before.st_mode))return 8;
        while((got=read(file,buffer,sizeof(buffer)))>0){ssize_t sent=0;while(sent<got){ssize_t n=write(STDOUT_FILENO,buffer+sent,(size_t)(got-sent));if(n<=0)return 8;sent+=n;}}
        {const char *barrier=getenv("WP_CODEBOX_READ_BARRIER");if(barrier&&*barrier){int marker=open(barrier,O_CREAT|O_EXCL|O_WRONLY|O_NOFOLLOW,0600);if(marker>=0){close(marker);while(access(barrier,F_OK)==0)usleep(1000);}}}
        if(got<0||fstat(file,&file_after)||!stable_metadata(file_before,file_after)||fstat(root,&root_after)||!stable_metadata(root_before,root_after)||!relative_file_identity(root,argv[3],identity_of(file_before)))return 8;
        return 0;
    }
    if(argc==6&&!strcmp(argv[1],"--cleanup")) {
        char *parent_path=NULL,*name=NULL;identity expected={(dev_t)strtoull(argv[3],NULL,10),(ino_t)strtoull(argv[4],NULL,10)};
        if(split_path(argv[2],&parent_path,&name))return 2;
        int parent=open_absolute_directory(parent_path);enum cleanup_result cleaned=parent<0?CLEAN_PENDING:cleanup_entry(parent,name,expected,argv[5]);
        free(parent_path);free(name);if(parent>=0)close(parent);
        if(cleaned==CLEAN_IDENTITY){write_status("RIGHT_OWNERSHIP_CHANGED");return 4;}
        if(cleaned==CLEAN_PENDING){write_status("CLEANUP_PENDING");return 7;}
        write_status("CLEANED");return 0;
    }
    if(argc!=5||strcmp(argv[1],"--transaction")){fprintf(stderr,"usage: atomic-directory-exchange --transaction <lock-dir> <left> <right>\n");return 2;}
    secure_lock_handle lock=secure_lock(argv[2]);if(lock.file<0){perror("secure promotion lock");return 1;}
    char *lparent_path=NULL,*rparent_path=NULL,*ln=NULL,*rn=NULL;
    if(split_path(argv[3],&lparent_path,&ln)||split_path(argv[4],&rparent_path,&rn))return 2;
    ...
    close(lock.file);close(lock.parent);free(lock.name);return 0;
}
