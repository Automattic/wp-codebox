...
async function main() {
  ...
  const bootstrapNeedle = "do_action('wp_codebox_sandbox_runtime_bootstrap'"
  ...
  assert.match(source, /wp_codebox_sandbox_runtime_context/, "sandbox boot should publish a generic mounted-runtime context")
  assert.ok(source.indexOf(bootstrapNeedle) < source.indexOf(pluginsLoadedNeedle), "mounted components should self-configure before generic lifecycle hooks")
