...
async function main() {
  ...
  assert.match(code, /<function_calls>/, "sandbox guidance should document Data Machine XML fallback syntax")
  assert.match(code, /<invoke name=\\"workspace_ls\\">/, "sandbox guidance should use supported invoke XML syntax")
  assert.match(code, /Do not emit direct XML tags such as <workspace_ls path=\\"/, "sandbox guidance should reject unsupported direct XML tags")
