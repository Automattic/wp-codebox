...
assert.match(projectBootstrapCode, /stream_get_wrappers\(\)/)
assert.match(projectBootstrapCode, /stream_wrapper_restore\('file'\)/)
assert.match(projectBootstrapCode, /'table_prefix' => \$bootstrap_mode === 'project' \? 'wp_' : 'wptests_'/)
assert.match(projectBootstrapCode, /WP_TESTS_SKIP_INSTALL=1/)
assert.match(projectBootstrapCode, /pg_run_project_bootstrap_stage/)
assert.match(projectBootstrapCode, /PROJECT_BOOTSTRAP:/)
assert.match(projectBootstrapCode, /\$pg_stage_output_buffering = true;\n\s*ob_start\(\);\n\s*require_once \$bootstrap_real;/)
assert.match(projectBootstrapCode, /\$pg_stage_output_buffering = false;\n\s*pg_stage_fail\('project_bootstrap', \$e\);/)
