...
async function assertPublicProvisioning(input) {
  ...
  if (operation?.state !== "succeeded" || imported?.status !== "imported" || imported.componentId !== "website-importer" || imported.importerVersion !== "1.3.6" || !imported.themeSlug
...
async function importStaticArtifact(input, expectedStatus = coordinator === "d1" ? 202 : 201) {
  ...
  if (expectedStatus !== 409 && (!result || typeof result !== "object" || result.status !== expectedResultStatus || result.componentId !== "website-importer" || result.importerVersion !== "1.3.6" || !result.themeSlug
