...
`wordpress.layout-sweep` measures caller-supplied containers and items across a continuous width range. It is not tied to a specific block. Pass `url`, `container-selector`, and `item-selector` (`:scope` is evaluated against each container). Optional inputs are `min-width` (320), `max-width` (1920), `profile=quick|deep`, `seed`, `concurrency`, `timeout`, and `scenarios`. Core scenarios are `sweep`, `history`, `storm`, `heights`, and `drag`. Perturbations are `text-scale:<percent>`, `block-fonts`, `long-text:<ratio>`, and `dpr:<n>`. Defaults are `sweep,history,storm,heights`. A width is settled when two consecutive animation frames report identical geometry; settling does not use timers. The quick profile sweeps a 16px grid, bisects mode or finding changes to 1px, and narrows height deviations until they are 2px apart. Container mode is the `--layout-mode` custom property, so a media query can mark a breakpoint without product-specific markup.

Finding kinds are `overflow` (text glyph extents past the item, measured with Range client rects), `leak` (text past the container), `overlap` (new relative to the widest width in the same mode), `collapsed`, `tiny-text`, `distort`, `hscroll`, `jump`, `hysteresis`, `history`, `storm`, `unstable`, and `error`. Page errors and console errors are findings; 4xx assets are listed separately by URL. `accepted` is a JSON array of `{kind, container, item}` identities to report without failing. The command emits `wp-codebox/layout-sweep/v1` plus `files/browser/layout-sweep/summary.json` and `files/browser/layout-sweep/findings.json`. Each group maps to a `homeboy/fuzz-finding/v1` record: identity is kind + container + item, with scenarios, count, width range, worst magnitude, sample, and replay args. `drag` reports CDP layout, style, and script time per step and does not fail the command. The same seed reproduces the same report.
