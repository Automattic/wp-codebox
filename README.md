...
- **Profile and prove performance fixes.** Capture real before/after evidence for a caching or query change instead of hand-waving. The same workflow backed three performance fixes that merged into WooCommerce core — coalescing redundant transient deletes ([#65591](https://github.com/woocommerce/woocommerce/pull/65591)), bounding layered-nav count cache growth ([#65552](https://github.com/woocommerce/woocommerce/pull/65552)), and fixing shipping-cache invalidation ([#65541](https://github.com/woocommerce/woocommerce/pull/65541)) — each one validated in a disposable runtime before opening the PR. WP Codebox can also throttle CPU/network to a deterministic low-end-mobile profile and assert web-performance budgets (`lcp_ms`, `fcp_ms`, `ttfb_ms`) against a real page load.
