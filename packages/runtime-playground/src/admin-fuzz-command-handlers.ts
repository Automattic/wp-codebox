...
foreach ($wp_codebox_admin_fuzz_targets as $wp_codebox_admin_fuzz_target) {
    ...
    $wp_codebox_admin_fuzz_coverage[] = array(
        ...
        'database' => array('status' => (string) ($wp_codebox_admin_fuzz_query_report['status'] ?? 'unavailable'), 'reason' => $wp_codebox_admin_fuzz_query_report['reason'] ?? null, 'queryCount' => (int) ($wp_codebox_admin_fuzz_query_report['queryCount'] ?? 0), 'totalTimeMs' => isset($wp_codebox_admin_fuzz_query_report['totalTimeMs']) ? round((float) $wp_codebox_admin_fuzz_query_report['totalTimeMs'], 3) : null, 'timingStatus' => (string) ($wp_codebox_admin_fuzz_query_report['timingStatus'] ?? 'unavailable'), 'timingReason' => $wp_codebox_admin_fuzz_query_report['timingReason'] ?? null, 'fingerprints' => is_array($wp_codebox_admin_fuzz_query_report['fingerprints'] ?? null) ? array_values($wp_codebox_admin_fuzz_query_report['fingerprints']) : array(), 'repeatedQueries' => is_array($wp_codebox_admin_fuzz_query_report['repeatedQueries'] ?? null) ? $wp_codebox_admin_fuzz_query_report['repeatedQueries'] : array(), 'writeSet' => is_array($wp_codebox_admin_fuzz_query_report['writeSet'] ?? null) ? $wp_codebox_admin_fuzz_query_report['writeSet'] : array(), 'repeatedWrites' => is_array($wp_codebox_admin_fuzz_query_report['repeatedWrites'] ?? null) ? $wp_codebox_admin_fuzz_query_report['repeatedWrites'] : array(), 'writeSetTruncated' => !empty($wp_codebox_admin_fuzz_query_report['writeSetTruncated'])),
    );
}
