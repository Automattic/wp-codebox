  ...
  return {
    ...
    properties: {
      ...
      adversarialCampaigns: {
        ...
        minItems: 1,
        maxItems: 32,
        items: { $ref: "#/$defs/adversarialRecipeCampaign" },
      },
      artifacts: {
        type: "object",
        additionalProperties: false,
    ...
    $defs: {
      ...
      stepPluginInput: {
        type: "object",
        additionalProperties: false,
        required: ["artifact", "packages"],
        description: "After this step succeeds, collect one declared typed JSON artifact, project it into immutable plugin package descriptors, resolve those packages on the host, and mount/activate them in the same runtime before later steps continue.",
        properties: {
          artifact: { type: "string", pattern: "^[A-Za-z0-9][A-Za-z0-9_.-]*$" },
          packages: { $ref: "#/$defs/pluginPackageProjection" },
        },
      },
      pluginPackageProjection: {
        ...
        additionalProperties: false,
        required: ["items", "map"],
        description: "Deterministic JSON Pointer projection from a consumer-owned typed artifact into generic immutable plugin package descriptors. Ambient stdout and key-name matching cannot supply package inputs.",
        properties: {
          resolver: { enum: ["immutable-archive", "wordpress.org-latest-stable"] },
      ...
      adversarialRecipeCampaign: {
        type: "object",
        additionalProperties: false,
        required: ["schema", "id", "seed", "corpus", "caseTemplates", "mutators", "oracles"],
        properties: {
          schema: { const: "wp-codebox/adversarial-recipe-campaign/v1" },
          id: { type: "string", pattern: "^[A-Za-z0-9][A-Za-z0-9_.-]*$" },
          seed: { type: "string", minLength: 1 },
          corpus: { type: "array", minItems: 1, maxItems: 1000, items: { $ref: "#/$defs/adversarialCorpusEntry" } },
          caseTemplates: { type: "array", minItems: 1, maxItems: 100, items: { $ref: "#/$defs/adversarialCaseTemplate" } },
          mutators: { type: "array", minItems: 1, uniqueItems: true, items: { enum: ["scalar", "structured", "binary", "sequence"] } },
          oracles: { type: "array", minItems: 1, maxItems: 32, items: { $ref: "#/$defs/adversarialOracle" } },
          matrix: { type: "array", maxItems: 16, items: { $ref: "#/$defs/adversarialMatrixDimension" } },
          concurrency: { type: "integer", minimum: 1, maximum: 128 },
          budgets: { $ref: "#/$defs/adversarialBudgets" },
          faultSchedule: { $ref: "#/$defs/transportFaultModel" },
          resetPolicy: { $ref: "#/$defs/adversarialResetPolicy" },
          requiredCapabilities: { type: "array", uniqueItems: true, items: { type: "string", minLength: 1 } },
          optionalCapabilities: { type: "array", uniqueItems: true, items: { type: "string", minLength: 1 } },
          novelty: { type: "object", additionalProperties: false, properties: { retainSignals: { type: "boolean" } } },
          shrinking: { type: "object", additionalProperties: false, properties: { enabled: { type: "boolean" } } },
          replay: { type: "object", additionalProperties: false, properties: { selection: { enum: ["findings", "all"] } } },
          metadata: { $ref: "#/$defs/metadata" },
        },
      },
      adversarialCorpusEntry: {
        ...
        properties: {
          id: { type: "string", pattern: "^[A-Za-z0-9][A-Za-z0-9_.-]*$" },
          actions: { type: "array", minItems: 1, maxItems: 10000, items: { $ref: "#/$defs/adversarialAction" } },
          input: {},
      ...
      adversarialCaseTemplate: {
        ...
        properties: {
          ...
          phases: {
            ...
            properties: {
              setup: { type: "array", items: { $ref: "#/$defs/step" } },
              action: { type: "array", items: { $ref: "#/$defs/step" } },
              assert: { type: "array", items: { $ref: "#/$defs/step" } },
              teardown: { type: "array", items: { $ref: "#/$defs/step" } },
            },
          },
      ...
      adversarialOracle: {
        ...
        properties: { id: { const: "runtime-status" }, severity: { enum: ["low", "medium", "high", "critical"] }, description: { type: "string" }, metadata: { $ref: "#/$defs/metadata" } },
      },
      adversarialMatrixDimension: {
        type: "object",
        additionalProperties: false,
        required: ["name", "values"],
        properties: { name: { type: "string", pattern: "^[A-Za-z0-9][A-Za-z0-9_.-]*$" }, values: { type: "array", minItems: 1, maxItems: 100, uniqueItems: true, items: { type: "string" } } },
      },
      adversarialBudgets: {
        type: "object",
        additionalProperties: false,
        properties: {
          maxCases: { type: "integer", minimum: 1, maximum: 100000 },
          maxActionsPerCase: { type: "integer", minimum: 1, maximum: 10000 },
          maxInputBytes: { type: "integer", minimum: 1, maximum: 67108864 },
          maxCaseTimeMs: { type: "integer", minimum: 1, maximum: 3600000 },
          maxWallTimeMs: { type: "integer", minimum: 1, maximum: 86400000 },
          maxArtifactBytes: { type: "integer", minimum: 1, maximum: 4294967296 },
        },
      },
      adversarialResetPolicy: {
        type: "object",
        additionalProperties: false,
        required: ["mode"],
        properties: {
          mode: { enum: ["none", "checkpoint-per-case", "restore-snapshot"] },
      ...
      transportFaultModel: {
        ...
        properties: {
          schema: { const: "wp-codebox/transport-fault-model/v1" },
          seed: { type: "string", minLength: 1 },
