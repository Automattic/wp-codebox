...
const browserActionsValidation: CommandValidationDescriptor = {
  requiredAnyArgs: [
    { names: ["steps-json", "url", "adaptive-exploration-json"], code: "missing-steps", message: "wordpress.browser-actions requires steps-json=<array>, url=<path-or-url>, or adaptive-exploration-json=<object>." },
  ],
  argRules: [
...
export const commandRegistry = [
  ...
  },
  {
    ...
    acceptedArgs: [
      ...
      { name: "adaptive-exploration-json", description: "Optional standalone wp-codebox/browser-adaptive-exploration/v1 mode. Repeatedly stabilizes and rediscovers unique controls while building a bounded deterministic state graph with replay, minimized finding evidence, and optional accessibility/keyboard/focus oracles; cannot be combined with url, steps-json, or action-corpus-json.", format: "JSON object" },
      ...
      { name: "transport-faults-json", description: "Optional canonical wp-codebox/transport-fault-model/v1 schedule applied only to HTTP(S) requests emitted by this isolated browser context. Server-side WordPress HTTP API requests are not intercepted.", format: "JSON object or @path" },
  ...
  },
  {
    ...
    acceptedArgs: [
      ...
      { name: "transport-faults-json", description: "Optional canonical wp-codebox/transport-fault-model/v1 schedule applied before navigation to HTTP(S) requests emitted by this isolated browser context. The same model may be supplied as scenario-json.transportFaults.", format: "JSON object or @path" },
  ...
  },
  {
    ...
    acceptedArgs: [
      ...
      { name: "matrix-json", description: "Optional comparison matrix object with comparisons and optional viewports arrays; each comparison may provide source/candidate targets, labels, viewport, wait settings (waitFor/wait-for and durationMs/duration), animatedMedia/animated-media, fullPage/full-page, threshold, includeAA/include-aa, maxRegions/max-regions, maxExplanationElements/maxExplanationCandidates/explainSelectors, or their kebab-case max-explanation-elements/max-explanation-candidates/explain-selector aliases. It also accepts reducedMotion/reduced-motion, animations, frozenTime/frozen-time, captureStyle/capture-style, and blockExternalRequests/block-external-requests.", format: "JSON object" },
      ...
      { name: "animated-media", description: "Allow animated images to play, or content-address and normalize supported animated images to a deterministic static first frame; defaults to allow.", format: "allow|first-frame" },
