...
import { BROWSER_SESSION_PRODUCT_DTO_SCHEMA, normalizeRuntimeAccess } from "./runtime-boundary-contracts.js";
export const METADATA_ARTIFACT_PATH = "metadata.json";
export const REVIEW_ARTIFACT_PATH = "files/review.json";
export const RUNTIME_EPISODE_TRACE_ARTIFACT_PATH = "files/runtime-episode-trace.json";
export const RUNTIME_EPISODE_EVENTS_ARTIFACT_PATH = "files/runtime-episode.jsonl";
export const RUNTIME_REFERENCE_MANIFEST_ARTIFACT_PATH = "files/runtime-reference-manifest.json";
export const RUNTIME_REPLAY_REFERENCE_INDEX_ARTIFACT_PATH = "files/runtime-replay-index.json";
export const RUNTIME_SNAPSHOT_ARTIFACT_PATH = "files/runtime-snapshot.json";
export const CHANGED_FILES_ARTIFACT_PATH = "files/changed-files.json";
export const PATCH_ARTIFACT_PATH = "files/patch.diff";
export const ARTIFACT_MANIFEST_PATH = "manifest.json";
export const PUBLIC_ARTIFACT_REF_DTO_SCHEMA = "wp-codebox/artifact-ref/v1";
const RUNTIME_REFERENCE_MANIFEST_EXCLUDED_PATHS = new Set([
...
const RUNTIME_REPLAY_REFERENCE_INDEX_EXCLUDED_PATHS = new Set([
    ARTIFACT_MANIFEST_PATH,
    RUNTIME_REPLAY_REFERENCE_INDEX_ARTIFACT_PATH,
]);
const BROWSER_ARTIFACT_SUMMARY_FIELDS = {
...
export function normalizeArtifactDigest(input) {
...
export function normalizeArtifactFileDigest(input) {
    return normalizeArtifactDigest(input);
}
export function normalizePublicArtifactRefDTO(input, defaults = {}) {
    const record = asRecord(input);
    if (!record) {
        return undefined;
    }
    const digest = normalizeArtifactDigest(record.digest)
        ?? normalizeArtifactDigest(record.sha256)
        ?? defaults.digest;
    ...
    return stripUndefined({
        ...
        contentType: normalizeArtifactContentType(record, defaults.contentType),
        sha256,
        digest,
...
export function normalizePublicArtifactRefDTOs(input) {
...
export function publicArtifactRefGroups(input) {
...
export function changedFilesArtifactRefs(input) {
    return publicArtifactRefGroups(input).changed_files;
}
export function patchArtifactRefs(input) {
    return publicArtifactRefGroups(input).patches;
}
export function findChangedFilesArtifactRef(input) {
    return changedFilesArtifactRefs(input)[0];
}
export function findPatchArtifactRef(input) {
    return patchArtifactRefs(input)[0];
}
export function normalizeBrowserSessionProductDTO(input) {
...
function browserSessionRuntimeAccess(session) {
...
export function normalizeArtifactContentType(input, fallback = "application/octet-stream") {
...
export function normalizeRuntimeReferenceManifestFileRef(input) {
...
export function normalizeRuntimeReferenceManifestFileRefs(inputs) {
    return inputs
        .map((input) => normalizeRuntimeReferenceManifestFileRef(input))
        .filter((ref) => ref !== undefined);
}
export function runtimeReferenceManifestArtifactFiles(files) {
    return files.filter((file) => !RUNTIME_REFERENCE_MANIFEST_EXCLUDED_PATHS.has(file.path));
}
export function runtimeReplayReferenceIndexArtifactFiles(files) {
    return files.filter((file) => !RUNTIME_REPLAY_REFERENCE_INDEX_EXCLUDED_PATHS.has(file.path));
}
export function normalizeRuntimeEpisodeTraceRef(input, defaults = {}) {
...
export function normalizeRuntimeEpisodeTraceRefs(inputs, defaults = {}) {
    return inputs
        .map((input) => normalizeRuntimeEpisodeTraceRef(input, defaults))
        .filter((ref) => ref !== undefined);
}
export function normalizeObservationArtifactRefs(input) {
    const refs = Array.isArray(input) ? input : input?.artifactRefs;
    return normalizeRuntimeEpisodeTraceRefs(refs ?? []);
}
export function normalizeArtifactBundleTraceRef(bundle) {
    if (!bundle) {
        return undefined;
    }
...
export function normalizeRuntimeReferenceArtifactBundleRef(input) {
    if (!input.id) {
        return undefined;
    }
...
export function normalizeBrowserArtifactSummaryRefs(summary) {
...
function collectArtifactRefCandidates(input) {
...
function appendArtifactRefArrayCandidates(candidates, value) {
...
function appendArtifactRefCandidate(candidates, value, defaults) {
    if (value !== undefined && value !== null) {
        candidates.push({ value, defaults });
    }
}
function appendPathCandidate(candidates, value, kind) {
    const path = stringValue(value);
    if (path) {
        candidates.push({ value: { path, kind }, defaults: { kind } });
    }
}
function uniquePublicArtifactRefs(refs) {
    const seen = new Map();
    const normalized = [];
...
function richerPublicArtifactRef(existing, incoming) {
    return stripUndefined({
        ...existing,
        ...incoming,
...
function isArtifactBundleRef(ref) {
    return ref.kind === "artifact-bundle" || ref.kind === "codebox-artifact-bundle";
}
function isChangedFilesArtifactRef(ref) {
    return ref.kind === "codebox-changed-files" || ref.kind === "changed-files" || pathEndsWith(ref.path, CHANGED_FILES_ARTIFACT_PATH);
}
function isPatchArtifactRef(ref) {
    return ref.kind === "codebox-patch" || ref.kind === "patch" || pathEndsWith(ref.path, PATCH_ARTIFACT_PATH);
}
function kindForArtifactPath(path) {
    if (pathEndsWith(path, CHANGED_FILES_ARTIFACT_PATH))
        return "codebox-changed-files";
    if (pathEndsWith(path, PATCH_ARTIFACT_PATH))
        return "codebox-patch";
    if (pathEndsWith(path, ARTIFACT_MANIFEST_PATH))
...
function pathIncludes(path, fragment) {
...
function compactPublicSessionValue(value, key = "") {
...
//# sourceMappingURL=artifact-references.js.map