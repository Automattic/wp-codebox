...
import type { RUNTIME_EPISODE_ACTION_SCHEMA, RUNTIME_EPISODE_OBSERVATION_SCHEMA, RUNTIME_EPISODE_SNAPSHOT_SCHEMA, RUNTIME_EPISODE_TRACE_SCHEMA } from "./runtime-episode-contracts.js";
...
export type BrowserStartupProgressPhase = "preview:start" | "preview:loading-client" | "preview:loading-wordpress" | "preview:applying-blueprint" | "preview:installing-dependencies" | "preview:activating-dependencies" | "preview:connecting-client" | "preview:ready" | "preview:error" | (string & {});
...
export interface WorkspaceRecipeAdversarialCampaign {
    ...
    corpus: Array<{
        ...
        actions: Array<{
            type: string;
            input?: unknown;
            clock?: import("./adversarial-campaign.js").AdversarialClockScheduleEntry[];
            metadata?: Record<string, unknown>;
        }>;
        input?: unknown;
        signals?: string[];
...
export interface ObservationSpec {
    type: "runtime-info" | "mounts" | "files" | "command-result" | "wordpress-state" | "http-response" | "browser-result" | "runtime-events" | "runtime-logs" | (string & {});
    path?: string;
    commandId?: string;
    url?: string;
    method?: string;
...
export interface LifecycleEvent {
    id: string;
    type: "runtime.created" | "runtime.mounted" | "runtime.command.started" | "runtime.command.finished" | "runtime.observed" | "runtime.snapshot.created" | "runtime.artifacts.collected" | "runtime.browser-startup-progress" | "runtime.destroyed" | (string & {});
    timestamp: string;
...
export declare class RuntimeCheckpointError extends Error {
    readonly diagnostic: RuntimeCheckpointFailureDiagnostic;
    constructor(diagnostic: RuntimeCheckpointFailureDiagnostic);
    toJSON(): RuntimeCheckpointFailureDiagnostic & {
        name: string;
    };
}
export declare function runtimeCheckpointUnsupportedDiagnostic(operation: RuntimeCheckpointOperation, runtime?: RuntimeInfo, name?: string): RuntimeCheckpointFailureDiagnostic;
...
export declare function createRuntime(spec: RuntimeCreateSpec, backend: RuntimeBackend): Promise<Runtime>;
export declare function restoreRuntime(snapshot: Snapshot, backend: RuntimeBackend, spec?: RuntimeRestoreSpec): Promise<Runtime>;
