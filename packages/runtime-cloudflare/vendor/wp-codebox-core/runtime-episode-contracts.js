...
export const RUNTIME_EPISODE_TRACE_SCHEMA = "wp-codebox/runtime-episode-trace/v1";
export const RUNTIME_EPISODE_ACTION_SCHEMA = "wp-codebox/runtime-episode-action/v1";
export const RUNTIME_EPISODE_OBSERVATION_SCHEMA = "wp-codebox/runtime-episode-observation/v1";
export const RUNTIME_EPISODE_SNAPSHOT_SCHEMA = "wp-codebox/runtime-episode-snapshot/v1";
...
export function runtimeEpisodeActionDigestPayload(action) {
    ...
    for (const key of ["cwd", "method", "url", "path", "operation", "selector", "description"]) {
...
export function runtimeEpisodeObservationDigestPayload(observation) {
    return {
...
export function runtimeEpisodeSnapshotDigestPayload(snapshot) {
    return {
...
export function validateRuntimeEpisodeTrace(trace) {
    const issues = [];
...
function validateRuntimeEpisodeStep(step, index, issues) {
...
function validateRuntimeEpisodeAction(action, path, issues) {
    if (!isRecord(action)) {
    ...
    for (const key of ["method", "url", "path", "operation", "selector", "description"]) {
    ...
    const expected = runtimeEpisodeDigest(runtimeEpisodeActionDigestPayload(action));
...
function validateRuntimeEpisodeObservation(observation, path, issues) {
    ...
    const expected = runtimeEpisodeDigest(runtimeEpisodeObservationDigestPayload(observation));
...
function validateRuntimeEpisodeSnapshot(snapshot, path, issues) {
    ...
    const expected = runtimeEpisodeDigest(runtimeEpisodeSnapshotDigestPayload(snapshot));
...
function validateRuntimeEpisodeTraceRef(ref, path, kind, issues) {
    if (!isRecord(ref)) {
...
function validateRuntimeEpisodeRefDigest(ref, targetDigest, path, issues) {
...
function collectForbiddenRuntimeEpisodeTraceFields(value, path, issues) {
...
function isRuntimeEpisodeFreeformPayloadPath(path) {
    return path === "$.artifacts" || path.endsWith(".data") || path.endsWith(".metadata");
}
function nonEmptyString(value) {
    return typeof value === "string" && value.length > 0;
}
//# sourceMappingURL=runtime-episode-contracts.js.map