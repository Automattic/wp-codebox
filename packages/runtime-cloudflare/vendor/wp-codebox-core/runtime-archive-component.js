...
export function runtimeArchiveComponentSource(value) {
  if (!isRecord(value) || value.schema !== RUNTIME_ARCHIVE_COMPONENT_SOURCE_SCHEMA || !isRecord(value.source)) throw new Error("Runtime archive component source is invalid.")
  let url
  try { url = new URL(String(value.source.url)) } catch { throw new Error("Runtime archive component source URL is invalid.") }
  if (url.protocol !== "https:" || url.username || url.password || !url.hostname || !isBoundedIdentity(value.source.version) || !isBoundedIdentity(value.source.identity) || !isSha256(value.source.sha256)) throw new Error("Runtime archive component source is invalid.")
  return {
...
function isAbility(value) { return typeof value === "string" && /^[a-z0-9]+(?:-[a-z0-9]+)*\/[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value) }
function isSha256(value) { return typeof value === "string" && /^[a-f0-9]{64}$/.test(value) }
function isBoundedIdentity(value) { return typeof value === "string" && value.length > 0 && value.length <= 256 && value.trim() === value }
function isSafeFilePath(value) { return typeof value === "string" && value.length > 0 && value.length <= 2_048 && !value.startsWith("/") && !value.endsWith("/") && !value.includes("\\") && value.split("/").every((segment) => segment !== "" && segment !== "." && segment !== "..") }
function isPluginInstallPath(value) { return typeof value === "string" && /^plugins\/[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value) }
function isMuPluginFilePath(value) { return isSafeFilePath(value) && /^mu-plugins\/[A-Za-z0-9._-]+\.php$/.test(value) }
