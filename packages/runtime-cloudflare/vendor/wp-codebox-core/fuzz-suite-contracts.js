...
import { fuzzCoveragePlanContract } from "./fuzz-coverage-plan-contracts.js";
export const FUZZ_SUITE_SCHEMA = "wp-codebox/fuzz-suite/v1";
export const FUZZ_SUITE_RESULT_SCHEMA = "wp-codebox/fuzz-suite-result/v1";
export const FUZZ_RUNNER_CAPABILITIES_SCHEMA = "wp-codebox/fuzz-runner-capabilities/v1";
export const FUZZ_RUNNER_READINESS_SCHEMA = "wp-codebox/fuzz-runner-readiness/v1";
export const FUZZ_ARTIFACT_BUNDLE_SCHEMA = "wp-codebox/fuzz-artifact-bundle/v1";
export const FUZZ_REPLAY_CASE_REF_SCHEMA = "wp-codebox/fuzz-replay-case-ref/v1";
export const FUZZ_MINIMIZE_CAPABILITY_SCHEMA = "wp-codebox/fuzz-minimize-capability/v1";
export const WORDPRESS_FUZZ_RUNTIME_CONTRACT_SCHEMA = "wp-codebox/wordpress-fuzz-runtime-contract/v1";
export const PHP_IN_PROCESS_FUZZ_SUITE_RUNNER_CAPABILITIES = {
...
export const WORDPRESS_FUZZ_RUNTIME_CONTRACT = {
...
export function wordpressFuzzRuntimeContract() {
...
export function fuzzSuiteResultEnvelope(input) {
    ...
    const status = summary.error > 0 || hasRequiredCoverageError || contractDiagnostics.length > 0 ? "error" : summary.failed > 0 ? "failed" : summary.skipped === summary.total && summary.total > 0 ? "skipped" : "passed";
    return stripUndefined({
        schema: FUZZ_SUITE_RESULT_SCHEMA,
...
export function fuzzReplayCaseRef(input) {
    return stripUndefined({
...
export function fuzzMinimizeUnsupportedCapability(input) {
    return stripUndefined({
        ...
        status: "unsupported",
        inputKind: "fuzz-replay-case",
        reason: input.reason,
        requiredArtifacts: input.requiredArtifacts,
        metadata: input.metadata,
    });
}
export function fuzzMinimizeSupportedCapability(input) {
    return stripUndefined({
        schema: FUZZ_MINIMIZE_CAPABILITY_SCHEMA,
        status: "supported",
        inputKind: "fuzz-replay-case",
...
export function fuzzArtifactBundleContract(input) {
    return stripUndefined({
...
export function fuzzSuiteRequiredRunnerCapabilities(suite) {
...
export function fuzzRunnerCapabilitiesContract(input, required) {
...
export function fuzzRunnerReadinessContract(input, required) {
    ...
    const status = unsupportedRequiredCapabilities.length > 0 ? "unsupported" : "ready";
    return stripUndefined({
        ...
        ...(input.mode === "runtime-backed" ? {
            disposable: true,
            isolation: { runtime_backed: true, disposable: true, sandboxed: true },
            guardrails: { external_side_effect_guardrail: true, external_http_guardrail: true },
            artifacts: { artifact_export: true },
            destructiveModeRequirements: WORDPRESS_FUZZ_RUNTIME_CONTRACT.destructiveModeRequirements,
        } : {}),
        metadata: input.metadata,
    });
}
export function unsupportedRequiredFuzzRunnerCapabilities(required, runnerCapabilities) {
    const requiredCapabilities = Array.isArray(required)
        ? [...required]
        : isFuzzSuiteContract(required)
            ? fuzzSuiteRequiredRunnerCapabilities(required)
...
function fuzzRunnerRequiredCapabilities(required, metadata) {
...
export function fuzzSuiteCaseResetPolicy(suite, fuzzCase) {
...
export function normalizeFuzzSuiteResetPolicy(input) {
...
export function fuzzSuiteResetPolicyDiagnostics(input, caseId) {
    const diagnostics = [];
...
function isFuzzSuiteContract(value) {
    return isRecord(value) && value.schema === FUZZ_SUITE_SCHEMA;
}
export function summarizeFuzzCases(cases) {
    const summary = { total: cases.length, passed: 0, failed: 0, error: 0, skipped: 0 };
...
function normalizeCaseResult(input) {
    return stripUndefined({
        id: input.id,
        status: input.status,
...
function fuzzSuiteResultCoveragePlan(suite, cases) {
...
function fuzzSuiteCoveragePlan(suite) {
    if ("coveragePlan" in suite && suite.coveragePlan?.schema === "wp-codebox/fuzz-coverage-plan/v1")
        return suite.coveragePlan;
    ...
    return metadataCoveragePlan?.schema === "wp-codebox/fuzz-coverage-plan/v1" ? metadataCoveragePlan : undefined;
}
function fuzzSuiteCaseCoveragePlanItem(fuzzCase) {
    return stripUndefined({
        id: fuzzCase.id,
...
function fuzzSuiteCaseResultCoveragePlanItem(result, base) {
    return stripUndefined({
        ...base,
        id: result.id,
...
function normalizeFuzzSuiteResetPolicyRecord(input) {
...
function isFuzzSuiteResetMode(mode) {
    return mode === "none" || mode === "checkpoint-per-case" || mode === "restore-snapshot";
}
function fuzzSuiteDiscoveredCount(suite, fallback) {
...
function fuzzSuiteAllowsEmpty(suite) {
    const metadata = fuzzSuiteMetadata(suite);
...
function fuzzSuiteExpectsNonEmptyCoverage(suite, coverageSummary) {
...
function fuzzSuiteRequiredArtifactDeclarations(suite) {
    const metadata = fuzzSuiteMetadata(suite);
    ...
    return candidates.filter((item) => isRecord(item) && item.required === true);
}
function fuzzSuiteMetadata(suite) {
...
function nonEmptyArrayField(source, key) {
    return arrayField(source, key).length > 0;
}
function stringArrayField(source, key) {
    return arrayField(source, key).filter((item) => typeof item === "string" && item.length > 0);
}
...
//# sourceMappingURL=fuzz-suite-contracts.js.map