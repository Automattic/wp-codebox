...
export const ADVERSARIAL_CAMPAIGN_SCHEMA = "wp-codebox/adversarial-campaign/v1";
export const ADVERSARIAL_CAMPAIGN_RESULT_SCHEMA = "wp-codebox/adversarial-campaign-result/v1";
export const ADVERSARIAL_FINDING_SCHEMA = "wp-codebox/adversarial-finding/v1";
export const ADVERSARIAL_REPLAY_SCHEMA = "wp-codebox/adversarial-replay/v1";
export const ADVERSARIAL_ORACLE_SCHEMA = "wp-codebox/adversarial-oracle/v1";
export const DIFFERENTIAL_RESULT_SCHEMA = "wp-codebox/differential-result/v1";
export function adversarialCampaign(input) {
    if (!input.id || !input.seed)
        throw new Error("Adversarial campaigns require non-empty id and seed values.");
    if (input.corpus.length === 0)
        throw new Error("Adversarial campaigns require at least one corpus entry.");
    const ids = new Set();
    for (const entry of input.corpus) {
        if (!entry.id || ids.has(entry.id))
            throw new Error(`Adversarial corpus ids must be non-empty and unique: ${entry.id}`);
        ids.add(entry.id);
    }
    return stripUndefined({
        ...
        mutationKinds: input.mutationKinds?.length ? [...new Set(input.mutationKinds)] : ["scalar", "structured", "binary", "sequence"],
...
export async function runAdversarialCampaign(campaignInput, options) {
    ...
    for (let round = 0; generated < campaign.budgets.maxCases; round += 1) {
        if (options.signal?.aborted) {
            incomplete = true;
            diagnostics.push({ code: "campaign-interrupted", message: "Campaign stopped after an interruption request." });
            break;
        }
        if ((options.now ?? Date.now)() - started >= campaign.budgets.maxWallTimeMs) {
            incomplete = true;
            diagnostics.push({ code: "campaign-wall-time-exhausted", message: "Campaign stopped at its wall-time budget." });
            break;
        }
        const roundPlans = [];
        for (let workerId = 0; workerId < campaign.budgets.workers && generated < campaign.budgets.maxCases; workerId += 1) {
            const corpusEntry = corpus[deterministicInteger(`${campaign.seed}:source:${generated}`, corpus.length)];
        ...
        for (let index = 0; index < roundPlans.length; index += 1) {
            ...
            if (artifactBytes > campaign.budgets.maxArtifactBytes) {
                incomplete = true;
                diagnostics.push({ code: "campaign-artifact-budget-exhausted", message: "Campaign stopped before artifact evidence exceeded its byte budget." });
                break;
            }
            const newSignals = (observation.signals ?? []).filter((signal) => !novelty.has(signal));
            for (const signal of newSignals)
                novelty.add(signal);
            if (newSignals.length > 0 && options.retainNovelty !== false)
                corpus.push({ id: plan.caseId, actions: plan.actions, input: plan.input, signals: [...new Set(observation.signals ?? [])], metadata: { retainedFrom: plan.corpusId, mutation: plan.mutation } });
            ...
            if (failedOracles.length > 0 || observation.status !== "passed") {
                if (options.signal?.aborted) {
                    incomplete = true;
                    diagnostics.push({ code: "campaign-interrupted", message: "Campaign stopped before finding minimization after an interruption request." });
                    break;
                }
                ...
                if (existing)
                    ...
                    findings.set(fingerprint, createFinding(campaign, plan, minimized, observation, failedOracles, schedule, options, fingerprint));
            }
        }
        if (incomplete)
            break;
...
export function mutateAdversarialValue(value, kind, seed) {
    if (kind === "sequence")
        return { value, path: "$", description: "sequence mutation is applied to actions" };
    const leaves = collectValueLeaves(value);
    if (leaves.length === 0)
        return { value: adversarialBoundaryValue(undefined, seed, kind), path: "$", description: `${kind} root boundary` };
    const leaf = leaves[deterministicInteger(seed, leaves.length)];
    const replacement = adversarialBoundaryValue(leaf.value, seed, kind);
    if (leaf.path.length === 0)
        return { value: replacement, path: "$", description: `${kind} root boundary mutation` };
...
export function adversarialFindingFingerprint(value) {
    return createHash("sha256").update("wp-codebox/adversarial-finding-fingerprint/v1\n").update(stableJson(value)).digest("hex");
}
export function classifyDifferentialResult(cells) {
    const fingerprints = [...new Set(cells.map((cell) => cell.fingerprint).filter((value) => Boolean(value)))].sort();
    ...
    if (cells.some((cell) => cell.status === "missing") || cells.some((cell, index) => cells.findIndex((candidateCell) => candidateCell.id === cell.id) !== index))
        classification = "nondeterminism";
    else if (candidate.some(isDifferentialFailure) && base.every((cell) => !isDifferentialFailure(cell)))
        classification = "candidate-regression";
    else if (candidate.some(isDifferentialFailure) && base.some(isDifferentialFailure))
        classification = "pre-existing";
    else if (fingerprints.length > 1 || new Set(cells.map((cell) => cell.status)).size > 1)
        classification = "platform-difference";
    return { schema: DIFFERENTIAL_RESULT_SCHEMA, classification, cells, fingerprints };
}
async function executeBoundedCase(campaign, plan, options) {
    ...
    try {
        ...
        const completed = await Promise.race([
            execution.then((observation) => ({ kind: "completed", observation })),
            new Promise((resolve) => { timer = setTimeout(() => { controller.abort(); resolve({ kind: "timed-out" }); }, campaign.budgets.maxCaseTimeMs); }),
        ]);
        ...
        const settled = await Promise.race([
            ...
            new Promise((resolve) => setTimeout(() => resolve(false), boundedInteger(options.abortSettleGraceMs, 1_000, 1, 60_000))),
        ]);
        if (!settled)
            return { status: "timed-out", diagnostics: [{ code: "case-timeout-unsettled", message: `Case exceeded ${campaign.budgets.maxCaseTimeMs}ms and did not settle during the abort grace period. Further cases require runtime teardown.` }], metadata: { cancellation: "runtime-teardown-required" } };
...
function mutateCorpusEntry(campaign, source, iteration, workerId) {
    const kind = campaign.mutationKinds[deterministicInteger(`${campaign.seed}:kind:${iteration}`, campaign.mutationKinds.length)];
    let actions = source.actions.map((action) => ({ ...action, input: cloneJsonValue(action.input) }));
    ...
    if (kind === "sequence") {
        ...
        actions[actionIndex] = { ...actions[actionIndex], input: result.value };
    ...
    if (jsonBytes(input) > campaign.budgets.maxInputBytes)
        input = truncateJsonValue(input, campaign.budgets.maxInputBytes);
...
async function minimizeAdversarialCase(campaign, plan, originalOracles, expectedFingerprint, options) {
    const oracleIds = new Set(originalOracles.filter((item) => item.failed).map((item) => item.oracleId));
    const preserves = async (candidate) => {
        if (options.signal?.aborted)
            return false;
    ...
    while (actions.length > 1 && chunk >= 1) {
        ...
        for (let start = 0; start < actions.length; start += chunk) {
            ...
            if (candidate.length === 0)
                continue;
            if (await preserves({ id: plan.caseId, actions: candidate, input: plan.input })) {
                actions = candidate;
                reduced = true;
                break;
            }
    ...
    for (let actionIndex = 0; actionIndex < actions.length; actionIndex += 1) {
        for (const candidateInput of shrinkAdversarialValue(actions[actionIndex]?.input)) {
            ...
            if (await preserves({ id: plan.caseId, actions: candidateActions, input: plan.input }))
                actions = candidateActions;
        }
    }
    let input = plan.input;
    for (const candidateInput of shrinkAdversarialValue(input)) {
        if (await preserves({ id: plan.caseId, actions, input: candidateInput }))
            input = candidateInput;
    }
    return stripUndefined({ id: plan.caseId, actions, input, metadata: { minimizedFrom: plan.corpusId } });
}
function createFinding(campaign, plan, minimized, observation, oracles, schedule, options, fingerprint) {
...
export function normalizeAdversarialClockSchedule(entries) {
    return entries.map((entry, index) => {
        if (!entry || !["runtime", "wordpress", "scheduler", "database"].includes(entry.surface))
            throw new Error(`Clock schedule entry ${index} has an invalid surface.`);
        if (!entry || !["freeze", "advance", "skew", "restore"].includes(entry.operation))
            throw new Error(`Clock schedule entry ${index} has an invalid operation.`);
        if ((entry.operation === "freeze" || entry.operation === "skew") && (!Number.isSafeInteger(entry.time) || entry.time < 0))
            throw new Error(`Clock schedule ${entry.operation} entry ${index} requires a non-negative Unix millisecond time.`);
        if (entry.operation === "advance" && (!Number.isSafeInteger(entry.milliseconds) || entry.milliseconds < 0))
            throw new Error(`Clock schedule advance entry ${index} requires non-negative milliseconds.`);
        if (entry.operation === "restore" && (entry.time !== undefined || entry.milliseconds !== undefined))
            throw new Error(`Clock schedule restore entry ${index} cannot include time values.`);
...
function defaultOracleResults(observation) {
...
function observationFingerprint(plan, observation, oracles) {
...
function mutateActionSequence(actions, seed, maximum) {
    if (actions.length === 0)
        return [{ type: "noop", metadata: { generated: true } }];
    const operation = deterministicInteger(seed, 4);
    const index = deterministicInteger(`${seed}:index`, actions.length);
    if (operation === 0 && actions.length > 1)
        return actions.filter((_, candidate) => candidate !== index);
    if (operation === 1 && actions.length < maximum)
        return [...actions.slice(0, index), actions[index], actions[index], ...actions.slice(index + 1)];
    ...
    return [...actions, actions[index]].slice(0, maximum);
}
function adversarialBoundaryValue(value, seed, kind) {
    if (kind === "binary") {
        const bytes = typeof value === "string" ? Buffer.from(value) : Buffer.from(stableJson(value));
        if (bytes.length === 0)
            return { encoding: "base64", data: "/w==" };
    ...
    if (typeof value === "number")
        return [0, -1, Number.MAX_SAFE_INTEGER, Number.MIN_SAFE_INTEGER][deterministicInteger(seed, 4)];
    if (typeof value === "boolean")
        return !value;
    if (value === null || value === undefined)
        return { unexpected: true };
    if (Array.isArray(value))
        return value.length === 0 ? [null] : [...value, cloneJsonValue(value[0])];
    if (typeof value === "object")
        return { ...value, __unexpected: { nested: true } };
    return String(value);
}
function shrinkAdversarialValue(value) {
    if (typeof value === "string")
        return ["", value.slice(0, Math.floor(value.length / 2)), value.slice(0, 1)];
    if (typeof value === "number")
        return [0, Math.sign(value)];
    if (Array.isArray(value))
...
function collectValueLeaves(value, path = []) {
    if (Array.isArray(value))
        return value.flatMap((item, index) => collectValueLeaves(item, [...path, index]));
    if (value && typeof value === "object")
        return Object.entries(value).flatMap(([key, item]) => collectValueLeaves(item, [...path, key]));
    return [{ path, value }];
}
...
function matrixCell(dimensions, iteration) {
    const cell = {};
    let divisor = 1;
    for (const dimension of dimensions ?? []) {
        if (dimension.values.length === 0)
            continue;
        cell[dimension.name] = dimension.values[Math.floor(iteration / divisor) % dimension.values.length];
        divisor *= dimension.values.length;
    }
    return cell;
}
function truncateJsonValue(value, maximumBytes) {
    if (typeof value === "string")
        return value.slice(0, maximumBytes);
...
function deterministicInteger(seed, maximum) {
    if (maximum <= 1)
        return 0;
...
function isDifferentialFailure(cell) {
    return cell.status !== "passed" && cell.status !== "missing";
}
//# sourceMappingURL=adversarial-campaign.js.map