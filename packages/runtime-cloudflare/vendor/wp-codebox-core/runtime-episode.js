...
import { ARTIFACT_MANIFEST_PATH, RUNTIME_EPISODE_EVENTS_ARTIFACT_PATH, RUNTIME_EPISODE_TRACE_ARTIFACT_PATH, RUNTIME_REFERENCE_MANIFEST_ARTIFACT_PATH, RUNTIME_REPLAY_REFERENCE_INDEX_ARTIFACT_PATH, normalizeArtifactBundleTraceRef, normalizeRuntimeReferenceArtifactBundleRef, normalizeRuntimeReferenceManifestFileRefs, runtimeReferenceManifestArtifactFiles, runtimeReplayReferenceIndexArtifactFiles, } from "./artifact-references.js";
import { isPlainObject as isRecord } from "./object-utils.js";
import { RUNTIME_EPISODE_ACTION_SCHEMA, RUNTIME_EPISODE_OBSERVATION_SCHEMA, RUNTIME_EPISODE_SNAPSHOT_SCHEMA, RUNTIME_EPISODE_TRACE_SCHEMA, runtimeEpisodeActionDigestPayload, runtimeEpisodeDigest, runtimeEpisodeObservationDigestPayload, runtimeEpisodeSnapshotDigestPayload, } from "./runtime-episode-contracts.js";
...
function observationWithId(observation, fallbackId) {
    const enveloped = {
        ...observation,
...
function snapshotWithSemantics(snapshot) {
    const enveloped = {
        ...snapshot,
...
function runtimeEpisodeJsonLines(trace) {
    const records = [
        {
            type: "episode.reset",
            id: trace.reset.id,
...
export async function createRuntimeEpisode(spec, backend) {
...
class RuntimeEpisodeRunner {
    ...
    static async create(spec, backend) {
    ...
    async step(action, observation = this.spec.stepObservation ?? false) {
    ...
    async restoreSnapshot(snapshotOrRef) {
    ...
    async collectArtifacts(spec = this.spec.artifactSpec ?? {}) {
    ...
    async persistRuntimeSnapshotBundles() {
        if (!this.artifacts || this.snapshots.length === 0) {
            return;
        }
        const manifest = JSON.parse(await readFile(this.artifacts.manifestPath, "utf8"));
    ...
    async persistRuntimeEpisodeTraceArtifacts() {
    ...
    async updateRuntimeReferenceManifestForRuntimeEpisodeTrace(traceRelativePath, eventsRelativePath) {
    ...
    async updateRuntimeReplayReferenceIndexForRuntimeEpisodeTrace(trace, traceRelativePath, eventsRelativePath) {
    ...
    async updateArtifactManifestForRuntimeEpisodeTrace(traceRelativePath, eventsRelativePath) {
        if (!this.artifacts) {
            return;
        }
        const manifest = JSON.parse(await readFile(this.artifacts.manifestPath, "utf8"));
    ...
    async updateArtifactMetadataForRuntimeEpisodeTrace(traceRelativePath, eventsRelativePath) {
        if (!this.artifacts) {
            return;
        }
        const metadata = JSON.parse(await readFile(this.artifacts.metadataPath, "utf8"));
        metadata.artifacts = {
    ...
    async updateArtifactReviewForRuntimeEpisodeTrace(traceRelativePath) {
        if (!this.artifacts) {
            return;
        }
        const review = JSON.parse(await readFile(this.artifacts.reviewPath, "utf8"));
...
async function createEpisodeRuntime(spec, backend) {
