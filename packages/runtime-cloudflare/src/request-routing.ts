...
export type EditorMemoryProbePhase = "admin" | "before-insert" | "after-insert" | "after-get-post" | "before-hooks" | "after-hooks" | "block-editor"
...
export function routeWorkerRequest(request: Request): WorkerRequestRoute {
  ...
  if (phase?.startsWith("editor-memory-")) {
    ...
    if (["admin", "before-insert", "after-insert", "after-get-post", "before-hooks", "after-hooks", "block-editor"].includes(editorPhase)) {
