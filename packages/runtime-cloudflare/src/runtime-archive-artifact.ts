...
export function validateRuntimeArchiveArtifactManifest(manifest: RuntimeArchiveArtifactManifest): void {
  ...
  if (manifest.component) {
    const component = runtimeArchiveComponent(manifest.component)
    if (manifest.name !== component.id) throw new Error("Runtime archive artifact name does not match its component.")
  }
}
...
import { runtimeArchiveComponent, type RuntimeArchiveComponent } from "./runtime-archive-component.js"
