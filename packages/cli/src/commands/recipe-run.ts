    ...
    await phaseTracker.run("run_workloads", phaseWorkflowData(workflowSteps), async () => {
      for (const workflowStep of workflowSteps) {
        ...
        try {
          ...
          if (workflowStep.step.command === "wordpress.collect-workload-result" && execution.exitCode === 0) {
            await awaitRecipe(`${operation}.materialize`, materializeCollectedWorkloadResult(runtime!, recipe.artifacts?.typed, workflowStep.step, execution))
          }
