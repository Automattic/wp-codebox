...
final class WP_Codebox_Runtime_Provider_Registry {
	...
	public static function resolve_runtime_requirements( array $input ): array {
		...
		$missing_adapters = self::missing_connector_adapters( $input );
		$installable      = array();
		...
		$available          = $provider_available && empty( $missing_adapters );
	...
	/** @param array<string,mixed> $input @return string[] */
	private static function missing_connector_adapters( array $input ): array {
		$missing = array();
		$inherit = is_array( $input['inherit'] ?? null ) ? $input['inherit'] : array();
		foreach ( is_array( $inherit['connectors'] ?? null ) ? $inherit['connectors'] : array() as $connector ) {
