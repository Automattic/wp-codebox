...
final class WP_Codebox_Agents_Md_Section {
	...
	public static function render(): string {
		...
		$lines = array(
			'## WP Codebox',
			'',
			'Disposable, isolated WordPress sites backed by WordPress Playground. Blow them up freely: CLI workloads, tests, fuzz, and browser/headless Playground sessions run here so the host install stays intact. Results return as artifact bundles the host reviews and applies back through the explicit apply-back path. Abilities: `wp-codebox/*`. CLI mirrors the same service layer.',
			'',
			'**Default routing**',
			...
			'',
			'**Safety**',
			'- Host apply-back uses the configured adapter. Review with `artifacts inspect`, then `preflight-apply`, then `apply` with `--approved-files` for the files you approved.',
			'- Provider credentials reach sandboxes through `secret_env` names. Keep secret values in the orchestrator environment; reference those names only in prompts, task payloads, logs, and artifacts.',
			'- WP-CLI runs in trusted operator context; shell access is the permission boundary.',
			'',
			'**Discovery**',
