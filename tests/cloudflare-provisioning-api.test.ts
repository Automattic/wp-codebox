...
function dynamicRuntime(db = database(), bucket = new Bucket(), maxSites = 128) {
  const value = runtime(db, bucket, undefined, maxSites)
  value.env.WORDPRESS_PREVIEW_DOMAIN = "preview.example"
  value.env.WORDPRESS_PREVIEW_HOST_SECRET = "preview-host-secret-with-at-least-thirty-two-bytes"
  return value
}
...
test("exact and concurrent same-key creates converge", async () => { const r = runtime(); const [one, two] = await Promise.all([create(r), create(r)]); assert.equal(one.status, 202); assert.equal(one.headers.get("x-wp-codebox-dispatch"), "backpressured"); assert.deepEqual(await one.json(), await two.json()); assert.equal(count(r.db, "wp_codebox_api_sites"), 1) })
test("accepted provisioning reports successful queue admission", async () => { const r = runtime(); r.env.WORDPRESS_RUNTIME_QUEUE = { async send() {} }; const response = await create(r); assert.equal(response.status, 202); assert.equal(response.headers.get("x-wp-codebox-dispatch"), "queued") })
...
test("dynamic preview allocation is unique, idempotent, and not limited by configured contexts", async () => {
  ...
  assert.equal(responses.filter((response) => response.status === 202).length, 100)
  const sites = await Promise.all(responses.map(async (response) => (await response.json() as { site: { id: string; url: string } }).site))
  assert.equal(new Set(sites.map((site) => site.id)).size, 100)
  assert.ok(sites.every((site) => site.url === `https://${site.id}.preview.example`))
  assert.equal(count(r.db, "wp_codebox_api_sites"), 100)
  const replay = await create(r, "dynamic-0")
  assert.deepEqual((await replay.json() as { site: { id: string } }).site.id, sites[0].id)
})
...
test("allocation recovery creates a missing claim before provisioning continues", async () => {
  const r = runtime(); const created = await (await create(r)).json() as { site: { id: string } }; r.db.sqlite.prepare("DELETE FROM wp_codebox_api_admin_claims").run(); await resumeProvisioningAllocation(r.env, { id: created.site.id, hostname: `${created.site.id}.example`, origin: `https://${created.site.id}.example` }, r.operations); assert.equal(count(r.db, "wp_codebox_api_admin_claims"), 1)
})
test("site resources expose administrator claim metadata without secrets", async () => {
  const r = runtime(); const created = await (await create(r)).json() as { site: { id: string; administratorClaim: { token: string } } }; const response = await routeProvisioningApi(new Request(`https://control.invalid/v1/sites/${created.site.id}`, { headers: { authorization: "Bearer good" } }), r.env, r.operations); const text = await response.text(); assert.match(text, /administratorClaim/); assert.doesNotMatch(text, new RegExp(created.site.administratorClaim.token)); assert.doesNotMatch(text, /capability_digest|password/)
})
test("administrator claim failures are generic, not-ready preserves pending, and expiry is terminal", async () => {
  const r = runtime(); const created = await (await create(r)).json() as { site: { id: string; administratorClaim: { token: string } } }; const url = `https://control.invalid/v1/sites/${created.site.id}/administrator-claim`
  for (const request of [new Request(url, { method: "POST" }), new Request(url, { method: "POST", headers: { authorization: "Bearer malformed" } }), new Request("https://control.invalid/v1/sites/beta/administrator-claim", { method: "POST", headers: { authorization: `Bearer ${created.site.administratorClaim.token}` } })]) { const response = await routeProvisioningApi(request, r.env, r.operations); assert.equal((await response.json() as { error: { code: string } }).error.code, "administrator_claim_unavailable") }
  const pending = await routeProvisioningApi(new Request(url, { method: "POST", headers: { authorization: `Bearer ${created.site.administratorClaim.token}` } }), r.env, r.operations); assert.equal(pending.status, 409); assert.equal((r.db.sqlite.prepare("SELECT state FROM wp_codebox_api_admin_claims").get() as { state: string }).state, "pending")
