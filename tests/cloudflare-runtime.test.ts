...
test("Cloudflare canonical runtime patches the unique init call with runtime persistence policies", async () => {
  ...
  for (const stage of ["boot-started", "boot-completed", "setup-completed", "files-collected", "revision-persisted", "lease-committed"]) assert.match(worker, new RegExp(`stage: "${stage}"`))
