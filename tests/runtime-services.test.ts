...
assert.equal(buildWordPressPhpunitRecipe({ pluginSlug: "example", databaseSetup: "external" }).runtime?.databaseSetup, "external")
assert.equal(validateWorkspaceRecipeJsonSchema({ schema: "wp-codebox/workspace-recipe/v1", runtime: { databaseSetup: "external" }, workflow: { steps: [{ command: "wordpress.run-php" }] } }).valid, true)
assert.equal(validateWorkspaceRecipeJsonSchema({ schema: "wp-codebox/workspace-recipe/v1", runtime: { databaseSetup: "implicit" }, workflow: { steps: [{ command: "wordpress.run-php" }] } }).valid, false)
