...
assert.equal(result.product_session.preview_boot.blueprint_ref.schema, "wp-codebox/browser-blueprint-ref/v1")
assert.equal(result.product_session.preview_boot.blueprint_ref.ref, `prepared:runtime-cache-key:${"a".repeat(64)}`)
assert.equal(result.product_session.preview_boot.blueprint_ref.hydration_status, "expired-transient")
assert.equal(result.product_session.preview_boot.blueprint_ref.hydratable, false)
assert.equal(result.product_session.preview_boot.blueprint_ref.hydrator_ability, "wp-codebox/hydrate-browser-blueprint-ref")
...
assert.equal(result.nested_primary_product_session.preview_boot.blueprint_ref.ref, `prepared:nested-cache-key:${"c".repeat(64)}`)
